...
export default function App() {
  const { isLoggedIn, user, logout, hydrateUser, fetchJournalTrades } = useAuth();
  useEffect(() => { hydrateUser(); }, [hydrateUser]);
  // ← ADD this entire block
  useEffect(() => {
    if (!isLoggedIn) { setDbTrades([]); return; }
    fetchJournalTrades()
      .then(data => {
  ...
  const [dbTrades, setDbTrades] = useState([]); // ← ADD
  ...
  const mergedTrades = useMemo(() => {
    const localKeys = new Set(
      trades.map(t => `${(t.scrip || "").toUpperCase()}|${t.boughtDate || ""}`)
    );
    const freshDbTrades = dbTrades.filter(
      dt => !localKeys.has(`${(dt.scrip || "").toUpperCase()}|${dt.boughtDate || ""}`)
    );
  ...
  return (
    <>
      ...
      <main className="page">
        {tab === "dashboard" && <Dashboard trades={mergedTrades} investments={investments} />}
        {tab === "journal" && <Journal trades={mergedTrades} onScripClick={setTradeDetail} />}
        ...
        {tab === "losing" && <Losing trades={mergedTrades} onScripClick={setTradeDetail} />}
