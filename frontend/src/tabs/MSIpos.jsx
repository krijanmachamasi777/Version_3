// src/tabs/MSIpos.jsx — Open IPOs/FPOs — uses AuthContext + correct /api/issues endpoint
import { useState, useEffect, useCallback } from "react";
import { useAuth } from "../context/AuthContext";
import "../styles/meroshare.css";

export function MSIpos() {
  const { fetchIssues, logout } = useAuth();
  ...
  const load = useCallback(() => {
    ...
    // fetchIssues calls GET /api/issues → data is an array of ApplicableIssue
    fetchIssues().then(d => setIssues(Array.isArray(d) ? d : [])).catch(e => setError(e.message)).finally(() => setLoading(false));
  }, [fetchIssues]);
    ...
    <div className="ms-wrap">
      ...
      <div className="card--np ms-card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>#</th><th>Script</th><th>Company</th><th>Type</th><th>Group</th><th>Open Date</th><th>Close Date</th></tr></thead>
            <tbody>
              {issues.length === 0 && <tr><td colSpan={7} className="td--empty">No open issues right now.</td></tr>}
              {issues.map((iss, i) => (
                <tr key={i}>
